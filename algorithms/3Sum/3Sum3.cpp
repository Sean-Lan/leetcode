...
class Solution {
    public:
        vector<vector<int>> threeSum(vector<int>& nums) {
            ...
            unordered_multiset<Triplet, TripletHash> resultSet;
            ...
            for (auto &num : nums) {
                ...
                if (numCnt.find(num)!=numCnt.end() && numCnt[num]==2) continue;
                ++numCnt[num];
                if (num < 0) {
            ...
            for (auto it = negatives.begin(); 
                ...
                for (auto it2 = next(it); it2!=negatives.end(); ++it2) {
                    ...
                    if (positives.find(num3) != positives.end()) {
                        Triplet triplet(num1, num2, num3);
                        if (resultSet.find(triplet)==resultSet.end()) {
