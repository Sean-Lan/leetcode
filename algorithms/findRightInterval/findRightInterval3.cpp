...
public:
    vector<int> findRightInterval(vector<Interval>& intervals) {
        ...
        vector<int> prevs; // the indexes of events which have ended
        for (int i=0; i<n; ++i) {
        ...
        for (auto &e : events) {
            if (e.isStart) {
                // update the right interval index
                // since this event must be the first one starts after those in prevs
                for (auto &i : prevs) result[i] = e.index;
                prevs.clear();
            } else { // end event
                prevs.push_back(e.index);
            }
        }
        return result;
    }
