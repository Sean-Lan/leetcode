...
// Just BFS, the unweighted will add serveral times according to the node's depth.
class Solution {
...
public:
    int depthSumInverse(vector<NestedInteger>& nestedList) {
        ...
        for (auto it=nestedList.begin(); it!=nestedList.end(); ++it) {
            if (it->isInteger()) unweighted += it->getInteger();
            else {
                aQueue.push({it, 2});
            }
        }
